...
# load fitted models
surv_fit = readRDS(here::here('02-data',
                              '03-results',
                              'real_fits',
                              'usgs',
                              'surv_fit_usgs.rds'))
...
samples_s = as_draws_df(surv_fit$draws())
samples_g = as_draws_df(growth_fit$draws())
samples_f = as_draws_df(fecd_fit$draws())
...
beta_s_names =   
  data.frame(
      param = c('beta[1]', 'beta[2]', 'beta[3]'),
      data_name = c('intercept', 'dbh_0.m', 'dbh2_0.m'),
      position = str_pad(1:3, width = 2, side = 'left', pad = '0'),
      pretty_name = 
          c('Intercept','DBH (m)', 'DBH^2 (m^2)'))
...
ggsave(sigmaPlot_s_priorvpost,
       filename = here::here('04-communication',
                  'figures',
                  'report',
                  'sigmas_s_priorvpost_usgs.png'), height = 4, width = 6.5, units = 'in')
...
ggsave(beta_g_priorvpost,
       filename = here::here('04-communication',
                             'figures',
                             'report',
                             'beta_g_priorvpost_usgs.png'), height = 4, width = 6.5, units = 'in')
...
ggsave(sigmas_g_priorvpost,
       filename = here::here('04-communication',
                             'figures',
                             'report',
                             'sigmas_g_priorvpost_usgs.png'), height = 4, width = 6.5, units = 'in')
...
ggsave(beta_f_priorvpost,
       filename = here::here('04-communication',
                             'figures',
                             'report',
                             'beta_f_priorvpost_usgs.png'), height = 4, width = 6.5, units = 'in')
...
ggsave(sigmaPlot_f_priorvpost,
       filename = here::here('04-communication',
                             'figures',
                             'report',
                             'sigmaPlot_f_priorvpost_usgs.png'),
       height = 4, width = 6.5, units = 'in')
...
summary_s = 
  surv_fit$summary(variables = c('beta', 'sigma_plot')) %>%
  left_join(beta_s_names %>%
              bind_rows(
                data.frame(param = c('sigma_plot'),
                       position = c('4'),
...
summary_g = 
  growth_fit$summary(variables = c('beta', 'sigma_plot',
                                 'sigma_epsilon')) %>%
  left_join(beta_g_names %>%
              bind_rows(
                data.frame(param = c('sigma_plot', 
...
summary_f = 
  fecd_fit$summary(variables = c('beta', 'sigma_plot', 'kappa')) %>%
  left_join(beta_f_names %>%
              bind_rows(
                data.frame(param = c('sigma_plot', 
                                     'kappa'),
