...
do.call('bind_rows',
  ...
  group_by(tree_id, dbh_0.m, surv_true) %>%
  summarise(p.50 = quantile(p, 0.5),
            p.mean = mean(p),
            p.025 = quantile(p, 0.025),
            p.975 = quantile(p, 0.975)) %>%
  ungroup() %>%
  mutate(dbh_class = cut(dbh_0.m, breaks = seq(from = 0.0254, to = 2.54, by = 0.0254),
                         labels = FALSE)) %>%
  group_by(dbh_class) %>%
...
samples.plotEffect_s = samples %>% select(contains('effect_plot'))
  ...
  do.call('bind_rows',
          lapply(X = 1:nrow(samples),
                 FUN = function(i){
                   ...
                   plot_effect = as.numeric(samples.plotEffect_s[i,])
                   
                   logf = 
                     as.numeric(pila_training$X %*% beta_f)+
                     plot_effect[pila_training$plot_id]
                   
                   f = 
                     matrix(nrow = pila_training$M, ncol = pila_training$C, 
                            byrow = FALSE,
                            data = 
                              sapply(X = 1:pila_training$C,
                                     FUN = function(census){
                                       sapply(X = 1:pila_training$M,
                                              FUN = function(sizeclass_from){
                                                exp(logf[sizeclass_from+(pila_training$M*(census-1))])
                                              })
                                     }))
                   nprime = 
                     sapply(X = 1:pila_training$C,
                   ...
                   result = 
                     data.frame(census = 1:pila_training$C) %>%
                     as_tibble()
