...
data {
  ...
  //array[N] int<lower=1,upper=P> ecosub_id; // plot indices
  matrix<lower=0>[M,P] n; // observed TPA of existing individuals for each size,census
  real a; // plot area searched for new recruits in hectares
...
model {
  ...
  // likelihood
  for (i in 1:P){
    cprime[i] ~ neg_binomial_2(nprime[i]*a, kappa);
  }
}