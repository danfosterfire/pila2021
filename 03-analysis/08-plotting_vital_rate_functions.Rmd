...
sigmaPlot_s_priorvpost = 
  ...
  theme_minimal()+
  theme(axis.text.y = element_blank())+
  labs(x = 'Plot Random Effect SD', y = 'Probability Density')
...
sigmaEco_s_priorvpost = 
  ...
  theme_minimal()+
  theme(axis.text.y = element_blank())+
  labs(x = 'Ecoregion Random Effect SD', y = 'Probability Density')
...
beta_g_priorvpost = 
  ...
  # instead of using simulated data to draw the prior, use the pdf directly
  # to appease reviewer comment about the scale on this figure
  geom_line(
    data = 
      
      beta_g_names %>%
      select(position, pretty_name, param) %>%
      left_join(
        betas_g %>%
      ...
      #) %>%
      mutate(x = value) %>%
      mutate(y = dnorm(x = x, mean = 0, sd = 10)),
    aes(x = x, y = y),
    color = 'red', lwd = 1
  )+
  theme_minimal()+
...
ggsave(beta_g_priorvpost,
       filename = here::here('04-communication',
                             'figures',
                             'manuscript',
                             'beta_g_priorvpost.png'), width = 10, height = 6, units = 'in')
...
sigmaPlot_g_priorvpost = 
  ...
  geom_line(
    data = 
      ...
      select(sigma_plot) %>%
      mutate(x = sigma_plot) %>%
      bind_rows(tibble(x = 0)) %>%
      mutate(y = truncnorm::dtruncnorm(x = x, mean = 0, sd = 5, a = 0)),
    aes(x = x, y = y),
    color = 'red', lwd = 1
  )+
  theme(axis.text.y = element_blank())+
...
sigmaEpsilon_g_priorvpost = 
  ...
  theme_minimal()+
  theme(axis.text.y = element_blank())+
  labs(x = 'Residual SD', y = 'Probability Density')
...
kappa_r_priorvpost = 
  ...
  labs(x = 'Kappa', y = 'Probability Density')
...
# and continuous variables held at 0 (other than size, they are scaled) 
  expand.grid('intercept' = 1,
            'dbh_m.init' = seq(from  = 0.0254, to = 1.25, by = 0.01),
            'fire' = 0,
            'wpbr' = 0,
            'ba_scaled' = 0,
            'cwd_dep90_scaled' = 0,
  ...
  summarise(p.median = median(p),
            p.05 = quantile(p, probs = 0.05),
            p.25 = quantile(p, probs = 0.25),
            p.75 = quantile(p, probs = 0.75),
            p.95 = quantile(p, probs = 0.95)) %>%
    ungroup() %>%
    select(dbh_m.init, p.median) %>%
    
    arrange(desc(p.median))
...
size_fixeff_s = 
  ...
  ggplot(data = .,
         aes(x = dbh_m.init))+
  geom_ribbon(aes(ymin = p.05, ymax = p.95),
              color = NA, alpha = 0.5)+
  geom_ribbon(aes(ymin = p.25, ymax = p.75),
              color = NA, alpha = 0)+
  geom_line(aes(y = p.median), lwd = 1)+
...
fire_fixeff_s = 
  ...
  geom_ribbon(aes(ymin = p.05, ymax = p.95, fill = as.factor(fire)),
              color = NA, alpha = 0.5)+
...
wpbr_fixeff_s = 
  ...
  geom_ribbon(aes(ymin = p.05, ymax = p.95, fill = as.factor(wpbr)),
              color = NA, alpha = 0.5)+
...
ba_fixeff_s = 
  ...
  geom_ribbon(aes(ymin = p.05, ymax = p.95, fill = as.factor(ba_scaled)),
              color = NA, alpha = 0.5)+
...
drought_fixeff_s = 
  ...
  geom_ribbon(aes(ymin = p.05, ymax = p.95, fill = as.factor(cwd_dep90_scaled)),
              color = NA, alpha = 0.5)+
...
dryness_fixeff_s = 
  ...
  geom_ribbon(aes(ymin = p.05, ymax = p.95, fill = as.factor(cwd_mean_scaled)),
              color = NA, alpha = 0.5)+
...
size_fixeff_g = 
  ...
  summarise(grow.median = median(grow),
            grow.025 = quantile(grow, probs = 0.05),
            grow.25 = quantile(grow, probs = 0.25),
            grow.75 = quantile(grow, probs = 0.75),
            grow.975 = quantile(grow, probs = 0.95)) %>%
  ggplot(data = .,
         aes(x = dbh_m.init))+
  ...
  geom_ribbon(aes(ymin = grow.25, ymax = grow.75),
              color = NA, alpha = 0)+
  geom_line(aes(y = grow.median), lwd = 1)+
  theme_minimal()+
  labs(y = 'DBH growth (m)',
       x = 'DBH (m)',
       title = 'a')+
...
size_fixeff_g+geom_hline(yintercept = 0.044)
