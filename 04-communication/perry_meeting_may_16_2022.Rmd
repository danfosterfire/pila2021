...
Is the wiggliness caused by jitter in the mean size for each bin? Would it 
be better to use midpoint sizes?
...
I expected it to be smoother? The vital rate functions are logistic-linear, 
linear, or log-linear. Somehow the combination of them is making that 
nonlinear looking pattern on the sensitivity? The median values are now in 
the range 0-1, but not all of the plot results are in the range 0-1.

This looks basically how you'd expect, in that its within-class transitions 
(survival and growth), esp of the larger classes, that matter most. But why 
only some of the larger classes, and not all of them?
...
write.csv(hypothetical_lambda_summary,
          here::here('04-communication', 
                     'tables',
                     'hypothetical_lambdas_summary_plots.csv'),
          row.names = FALSE)
...
An individual plot's sensitivity matrix:
...
Still getting some weird results with the truncated size range.

# Hypothetical plots, limited size classes TODO RUN THIS
...
# Problem is mean size rule?


# Real plots, full size range, midpoint rule
...
# save this so we don't have to rebuild it every time we knit
saveRDS(A_median_full_midpoint,
        here::here('02-data',
                   '03-results',
                   'real_fits',
                   'A_median_full_midpoint.rds'))
...
```{r}
lambda_full_midpoint = 
  sapply(X = 1:nrow(plots.pila),
         FUN = function(plot){
           A_plot = A_median_full_midpoint[,,plot]
           lambda_plot = max(as.numeric(Re(eigen(A_plot)$values)))
           return(lambda_plot)
         })

ggplot(data.frame(lambda_full_midpoint),
       aes(x = lambda_full_midpoint))+
  geom_density()+
  theme_minimal()

ggplot(data.frame(lambda_full_midpoint),
       aes(x = log(lambda_full_midpoint)))+
  geom_density()+
  theme_minimal()
...
# proportion of plots where lambda < 1
length(lambda_full_midpoint[lambda_full_midpoint<1])/length(lambda_full_midpoint)

quantile(lambda_full_midpoint, c(0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95))
```
...
ssd_full_midpoint = 
  matrix(nrow = nrow(size_metadata),
         ...
         data = 
           sapply(X = 1:nrow(plots.pila),
                  FUN = function(plot){
                    A.plot = A_median_full_midpoint[,,plot]
                    # from supplamentory materials for merow et al 2014 
                    # "On using integral projection models..."
                    w.eigen = Re(eigen(A.plot)$vectors[,1])
                    ssd = w.eigen / sum(w.eigen)
...
ssd_full_midpoint.df$class_proportion = 
  
  as.numeric(
    sapply(X = 1:nrow(size_metadata),
           FUN = function(sizeclass){
             
             return(ssd_full_midpoint[sizeclass,])
...
plot_repro_full_midpoint.df$reproductive_value = 
...
```{r}
plot_repro_full_midpoint.df  %>% summary()
```
...
```{r}
# sensitivity and elasticity
v.dot.w_plot_full_midpoint = 
  
  sapply(X = 1:nrow(plots.pila),
         FUN = function(plot){
           sum(ssd_full_midpoint[,plot] * plot_repro.full[,plot])*0.127
         })

sens_plot_full_midpoint = 
  array(dim = list(nrow(size_metadata),
                   nrow(size_metadata),
                   nrow(plots.pila)),
        dimnames = 
        ...
        data = 
          sapply(X = 1:nrow(plots.pila),
                 FUN = function(plot){
                   outer(plot_repro.full[,plot], ssd_full_midpoint[,plot])/
                     v.dot.w_plot_full_midpoint[plot]
                 }))


elas_plot_full_midpoint = 
  ...
  array(dim = list(nrow(size_metadata),
        ...
        data = 
          sapply(X = 1:nrow(plots.pila),
                 FUN = function(plot){
                   matrix(as.vector(sens_plot_full_midpoint[,,plot])*
                            as.vector(A_median_full_midpoint[,,plot])/
                            lambda_full_midpoint[plot])
                 }))
...
sens_elas_plot_full_midpoint.df = 
  expand.grid(size_to = size_metadata$bin_id,
              size_from = size_metadata$bin_id) %>%
  left_join(size_metadata %>%
              select(size_to = bin_id, 
...
sens_elas_plot_full_midpoint.df$sensitivity = 
                  ...
                  FUN = function(size_to){
                    ...
                    sapply(X = 1:nrow(size_metadata),
                           FUN = function(size_from){
                             ...
                             sens_plot_full_midpoint[size_to, size_from, plot]
...
```{r}
summary(sens_elas_plot_full_midpoint.df)
```
...
fields::image.plot(size_metadata$bin_midpoint,
                   size_metadata$bin_midpoint,
                   t(A_median_full_midpoint[,,571]),
                   xlab = 'Size (t)', ylab = 'Size (t+1)')

fields::image.plot(size_metadata$bin_midpoint,
                   size_metadata$bin_midpoint,
                   t(A_median_full_midpoint[,,669]),
                   xlab = 'Size (t)', ylab = 'Size (t+1)')
```
...
fecundity function has though.
```{r}
sens_elas_plot_full_midpoint.df$elasticity = 
                  ...
                  FUN = function(size_to){
                    ...
                    sapply(X = 1:nrow(size_metadata),
                           FUN = function(size_from){
                             ...
                             elas_plot_full_midpoint[size_to, size_from, plot]
...
saveRDS(A_hypotheticals_full_midpoint,
        here::here('02-data',
                   '03-results',
                   'real_fits',
                   'hypothetical_As_full_midpoint.rds'))
...
hypothetical_lambdas_full_midpoint.matrix = 
  matrix(nrow = nrow(hypothetical_plots),
         ncol = 4000,
         data = 
           sapply(X = 1:nrow(hypothetical_plots),
                  FUN = function(plot){
                    sapply(X = 1:4000,
                           FUN = function(draw){
                             max(Re(eigen(A_hypotheticals_full_midpoint[,,plot,draw])$values))
                           })
                  }))

hypothetical_lambdas_full_midpoint = 
  hypothetical_plots %>%
  expand(nesting(subp_id, name, intercept, fire, wpbr, ba_scaled, 
                 cwd_dep90_scaled, cwd_mean_scaled),
         data.frame(draw = 1:4000))
...
hypothetical_lambdas_full_midpoint$lambda = 
  sapply(X = 1:nrow(hypothetical_plots),
         FUN = function(plot){
           sapply(X = 1:4000,
                  FUN = function(draw){
                    # paste0('s:',plot,'d:',draw) for testing
                    max(as.numeric(Re(eigen(A_hypotheticals_full_midpoint[,,plot,draw])$values)))
                  })
         }) %>%
  as.numeric()
...
```{r}
hypothetical_ssd_full_midpoint = 
  array(dim = c(nrow(size_metadata),
                nrow(hypothetical_plots),
                4000),
        dimnames = 
        ...
        data = 
          sapply(X = 1:4000,
                 FUN = function(draw){
                   sapply(X = 1:nrow(hypothetical_plots),
                          FUN = function(plot){
                            A = A_hypotheticals_full_midpoint[,,plot,draw]
                            w.eigen = Re(eigen(A)$vectors[,1])
                            ssd = w.eigen / sum(w.eigen)
                            return(ssd)
                          })
...
hypothetical_ssd_full_midpoint.df = 
  hypothetical_plots %>%
  expand(nesting(intercept, fire, wpbr, ba_scaled, cwd_dep90_scaled,
                 cwd_mean_scaled, subp_id, name),
         sizeclass = 1:nrow(size_metadata),
...
hypothetical_ssd_full_midpoint.df$class_proportion = 
             ...
             as.numeric(
               sapply(X = 1:nrow(size_metadata),
                      FUN = function(sizeclass){
                        ...
                        return(hypothetical_ssd_full_midpoint[sizeclass,plot,])
                      })
             )
...
hypothetical_repro_full_midpoint = 
  array(dim = c(nrow(size_metadata),
                nrow(hypothetical_plots),
                4000),
        dimnames = 
...
hypothetical_repro_full_midpoint.df = 
  hypothetical_plots %>%
  expand(nesting(intercept, fire, wpbr, ba_scaled, cwd_dep90_scaled,
                 cwd_mean_scaled, subp_id, name),
         sizeclass = 1:nrow(size_metadata),
...
hypothetical_repro_full_midpoint.df$repro = 
             ...
             as.numeric(
               sapply(X = 1:nrow(size_metadata),
                      FUN = function(sizeclass){
                        ...
                        return(hypothetical_repro_full_midpoint[sizeclass,plot,])
                      })
             )
...
# sensitivity and elasticity
hypothetical_vdotw_full_midpoint = 
  matrix(nrow = nrow(hypothetical_plots),
         ncol = 4000,
         data = 
           sapply(X = 1:nrow(hypothetical_plots),
                  FUN = function(plot){
                    sapply(X = 1:4000,
                           FUN = function(draw){
                             sum(hypothetical_ssd_full_midpoint[,plot,draw] *
                                   hypothetical_repro_full_midpoint[,plot,draw])*0.127
                           })
                  }))


hypothetical_sens_full_midpoint = 
  array(dim = list(nrow(size_metadata),
                   nrow(size_metadata),
                   nrow(hypothetical_plots),
                   4000),
        ...
        data = 
          ...
          sapply(X = 1:4000,
                 FUN = function(draw){
                   sapply(X = 1:nrow(hypothetical_plots),
                          FUN = function(plot){
                            outer(hypothetical_repro_full_midpoint[,plot,draw], 
                                  hypothetical_ssd_full_midpoint[,plot,draw])/
                              hypothetical_vdotw_full_midpoint[plot,draw]
                          })
                 })
  )

hypothetical_elas_full_midpoint = 
  array(dim = list(nrow(size_metadata),
                   nrow(size_metadata),
                   nrow(hypothetical_plots),
                   4000),
                 ...
                 FUN = function(draw){
                   sapply(X = 1:nrow(hypothetical_plots),
                          FUN = function(plot){
                            ...
                            matrix(as.vector(hypothetical_sens_full_midpoint[,,plot,draw])*
                                     as.vector(A_hypotheticals_full_midpoint[,,plot,draw])/
                                     hypothetical_lambdas_full_midpoint.matrix[plot,draw])
...
hypothetical_sens_elas_full_midpoint.df  = 
  expand.grid(size_to = size_metadata$bin_id,
              size_from = size_metadata$bin_id) %>%
  left_join(size_metadata %>%
              select(size_to = bin_id, 
...
hypothetical_sens_elas_full_midpoint.df$sensitivity = 
  sapply(X = 1:nrow(hypothetical_plots),
         FUN = function(plot){
           sapply(X = 1:nrow(size_metadata),
                  FUN = function(size_to){
                    ...
                    sapply(X = 1:nrow(size_metadata),
                           FUN = function(size_from){
                             hypothetical_sens_full_midpoint[size_to, size_from, plot,]
                           })
                  })
         }) %>%
  as.vector()
...
hypothetical_sens_elas_full_midpoint.df$elasticity = 
  sapply(X = 1:nrow(hypothetical_plots),
         FUN = function(plot){
           sapply(X = 1:nrow(size_metadata),
                  FUN = function(size_to){
                    ...
                    sapply(X = 1:nrow(size_metadata),
                           FUN = function(size_from){
                             hypothetical_elas_full_midpoint[size_to, size_from, plot,]
                           })
                  })
         }) %>%
  as.vector()
...
```{r}
hypothetical_sens_elas_full_midpoint.df$transition = 
    sapply(X = 1:nrow(hypothetical_plots),
         FUN = function(plot){
           sapply(X = 1:nrow(size_metadata),
                  FUN = function(size_to){
                    ...
                    sapply(X = 1:nrow(size_metadata),
                           FUN = function(size_from){
                             A_hypotheticals_full_midpoint[size_to, size_from, plot,]
                           })
                  })
         }) %>%
  as.vector()
...
# Real plots, limited size range, midpoint
...
# Hypothetical plots, limited size classes, midpoint
