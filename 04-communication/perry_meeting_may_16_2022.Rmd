...
Here's where things start getting really wonky; there are subplot:sizeclass 
...
the range 0-1, but not all of the subplot results are in the range 0-1.
...
Results are similar, though here 80% of subplots have a lambda below one 
rather than 59%.
...
Have some NA, Inf, and -Inf values. Overflow?
...
This looks basically reasonable, but there's **still** a bunch (1119 / 44080) of 
subplot:sizeclass combinations where we have a have a negative reproductive 
value, sometimes with large magnitude.

```{r}
nrow(subplot_repro_half.df %>% filter(reproductive_value<0))
...
Again getting elasticity values outside the range 0-1, and the NA subplots:
